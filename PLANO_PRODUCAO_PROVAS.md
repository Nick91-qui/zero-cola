...
- [ ] Expor listagem de questões com filtros para o professor.
- [ ] Expor detalhe de questão com habilidades associadas.
- [ ] Permitir paginação e busca por texto, habilidade e status.
- [ ] Criar testes para a visualização do Banco de Questões.

### 3.2 Montagem de prova por seleção
- [ ] Separar a UI de criação da prova da UI de criação de questão.
- [ ] Permitir adicionar/remover questões já existentes em uma prova.
- [ ] Permitir ordenar questões e ajustar pesos antes da publicação.
- [ ] Preservar Workflow B para provas sem Banco de Questões.
...
- [ ] Gerar uma folha personalizada por aluno.
- [ ] Preencher nome, código do aluno e identificadores necessários.
- [ ] Manter a compatibilidade com o fluxo OMR existente.
- [ ] Criar testes de regressão para a geração individual e em lote.

### 3.5 Validação e regressão
- [ ] Garantir que Workflow A e Workflow B continuem funcionando.
- [ ] Garantir que tentativas online continuem imunes a vazamento de gabarito.
- [ ] Garantir que a correção OMR continue usando `AnswerKeyItem.correct_answer`.
- [ ] Garantir que turmas e matrículas sejam respeitadas na geração das folhas.
...
Esta etapa só pode ser considerada concluída quando:

1. o professor puder montar uma prova selecionando questões existentes;
2. o professor puder visualizar a prova antes de publicar;
3. a publicação continuar gerando `AnswerKey` imutável;
4. folhas OMR personalizadas puderem ser geradas por aluno;
5. o fluxo respeitar turmas, matrículas e RBAC;
6. Workflow A, Workflow B, online e OMR continuarem funcionando;
7. testes automatizados cobrirem visualização, montagem, preview e geração personalizada;
8. a documentação do roadmap e do status refletir este novo passo.
