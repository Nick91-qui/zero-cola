...
export default function ClassDetailPage() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- data fetch hydrates local class state on mount
    void loadClass();
  }, [loadClass]);
    ...
    <ProtectedRoute requiredRoles={['teacher', 'admin']}>
      <div className="min-h-screen bg-slate-50">
        ...
        <main className="mx-auto max-w-6xl px-4 py-10">
          ...
          {loading ? (
            <p className="mt-8 text-center text-sm text-slate-500">Carregando turma...</p>
          ) : error || !classData ? (
                  ...
                  {classData.teachers.length === 0 ? (
                    ...
                    <div className="mt-6 space-y-3">
                      {classData.teachers.map((membership) => (
                        <article key={membership.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <article key={membership.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              <p className="font-semibold text-slate-900">
                                {membership.student
                                  ? formatName(membership.student.email, membership.student.student_code)
